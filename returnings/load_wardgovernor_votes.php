             ...
             if ($result1->num_rows > 0) {
              while($row1 = $result1->fetch_assoc()) {
                ...
                $wardcode=$row1['wardcode'];
                          ...
                          ?>
                            <div class="well btn pull-left" style="width: 32%;margin-left: 5px;color: black;min-width: 250px;background-color: #cfd2d6;">
                            <h4 class="badge"><?php echo $row1['wardname'];?></h4>
                            <div class="panel">
                ...
                <?php
                        ...
                        $sql2 = "SELECT * FROM `ward_results` where ward='$wardcode' && type='Governor'";
                         $result2 = $conn->query($sql2);
                         if ($result2->num_rows > 0) {
                          while($row2 = $result2->fetch_assoc()) {
                            $pres=$row2['contestant'];

                               $sql21 = "SELECT * FROM governor where Sno='$pres' ORDER BY Sno ";
                                  ...
                                  if(mysqli_num_rows($resultvote)>0){
                                    while($row21 =mysqli_fetch_array($resultvote)) {
                                       echo '<tr><td>' . $row21["uname"]. '</td><td> <img width="40px" height="30px" src="../uploads/'.$row21['photo'].'" /></td><td>'.$row2['votes'].'</td></tr>';
                                           ...
                                           echo '<tr><td colspan="3">Governor not Found</td></tr>';
                                           
                                        }
                                  }
                                }
                                else{
                                          echo '<tr><td colspan="3">No Vote For Any Governor in this ward</td></tr>';
                                     }
