 ...
 $this->SetDrawColor(0,80,180);
   ...
   // $this->Cell(2);
    ...
    $this->SetFont('Times','',20);
     ...
     $this->Cell(0,20,'Senatorial Results ',0,1,'C');
     ...
     $this->Cell(10,10,'Sno',1,0,'C');
       ...
       $this->SetTextColor(0,255,0);
     $this->SetFont('Times','',13);
     $this->Cell(70,10,'Senator Name',1,0,'C');
...
$counts=1;
                        $sql2 = "SELECT * FROM `constituency_results` where constituency='$current_constituency' && type='Senator'";
                         $result2 = $conn->query($sql2);
                         if ($result2->num_rows > 0) {
                          while($row2 = $result2->fetch_assoc()) {
                            $pres=$row2['contestant'];

                               $sql21 = "SELECT senator.Sno,senator.uname,senator.partycode,party.partycode,party.partyname FROM `senator`,party WHERE senator.Sno='$pres' && senator.partycode=party.partycode order by senator.Sno  ";
                           ...
                           $counts++;
                                           ...
                                           $pdf->Cell(100,10,'Senator not Found ',1,1,'C');
                                           
                                        }
                                  }
                                }
                                else{
                                         $pdf->Cell(100,10,'No Vote For Any Senator '.mysqli_error($connection),1,1,'C');
                                     }
  

                         $sql21 = "SELECT senator.Sno,senator.uname,senator.partycode,party.partycode,party.partyname,constituency_results.type,max(constituency_results.votes) as winner,constituency_results.contestant FROM `senator`,party,constituency_results WHERE  senator.partycode=party.partycode && constituency='$current_constituency' && type='Senator' order by senator.Sno  ";
                      ...
                      if(mysqli_num_rows($resultvote)>0){
                        while($row21 =mysqli_fetch_array($resultvote)) {
                         ...
                         //  $pdf->Cell(70,6,mysqli_num_rows($resultvote),1,0,'C');
                                         ...
                                         $pdf->Cell(100,10,'No Vote For Any Senator '.mysqli_error($conn),1,1,'C');
                                     }
             $conn->close();
