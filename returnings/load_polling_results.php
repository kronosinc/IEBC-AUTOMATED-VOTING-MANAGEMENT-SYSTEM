<div class="box-header" style="background-image: url('../dist/img/boxed-bg.jpg');background-repeat: no-repeat;  background-size: cover;background-position: center;opacity:0.9;">
              <i class="fa fa-graduation-cap"></i>

              <h3 class="box-title glyphicon glyphicon-info-sign" style="color:green;"></h3><b> Polling Station Results Information</b>
              <h3 class="box-title glyphicon glyphicon-info-sign" style="float:right;color:green;"></h3><b  style="float:right;"> Info<h style="color:gray;">_</h> </b>

            </div>


             <!-- lkkjuu -->
            <div class=" pull-left notifications well" style="width: 100%;margin-top: -1.5%;">
                    <ul id="myTab" class="nav nav-tabs">
                       <li class="active"><a href="#viewpresidentvotes" data-toggle="tab">President</a></li>
                        <li><a href="#viewgovernorvotes" data-toggle="tab">Governor</a></li>
                       <li><a href="#viewsenatorvotes" data-toggle="tab">Senator</a></li>
                        <li > <a href="#viewwomenrepvotes" data-toggle="tab">Women Rep </a></li>
                       <li><a href="#viewmpvotes" data-toggle="tab">Member Of Parliament</a></li>
                       <li><a href="#viewmcavotes" data-toggle="tab">MCA </a></li>
                 </ul>
            <div id="myTabContent" class="tab-content" style="margin-top: 1%;">
          

                <div class="tab-pane fade in active" id="viewpresidentvotes" style="margin-top: -1%;">
                <!-- presidential results panel -->
                 <div class="btn btn- well pull-left"  style="color:black;width: 100%;min-width:240px;margin-left: 10px;padding: 1px;">
                  <div class="panel">
                   <h5 style="text-align: center;font-size: 20px;" >Presidential Results  <h class="pull-right" style="margin-top: -0.5%;padding-right: 2px;"> <div class="pull-right" >
                        <a href="printpresident34B.php" class="" style="font-size: 10px;margin: 0px;" ><button class="btn btn-large btn-info printpresident box-title glyphicon glyphicon-print bg-aqua" style="margin: 0px;"><h style="color: yellow;"><b> Print </b></h></button> </a>
                     
                    </div></h></h5>
                   
                          <div class="presidentialpollingresults" id="presidentialpollingresults">
                              No Voting Activity Detected
                          </div>
                 </div>
             
                </div>
                  <!-- end of presidential results  -->
                    
                </div>
                <div class="tab-pane fade" id="viewgovernorvotes" style="margin-top: -1%;">
                 <!-- governor results panel -->
                <div class="btn btn- well pull-left"  style="color:black;width: 100%;min-width:240px;margin-left: 10px;padding: 3px;">
                         <div class="panel">
                       <h5 style="text-align: center;font-size: 20px;" >Gobanatorial Results  <h class="pull-right" style="margin-top: -0.5%;padding-right: 2px;"> <div class="pull-right" >
                        <a href="printpresident34B.php" class="" style="font-size: 10px;margin: 0px;" ><button class="btn btn-large btn-info printpresident box-title glyphicon glyphicon-print bg-aqua" style="margin: 0px;"><h style="color: yellow;"><b> Print </b></h></button> </a>
                     
                    </div></h></h5>
                      <div class="governorpollingresults" id="governorpollingresults">
                                  No Voting Activity Detected
                              </div>
                       
                 
                    </div>
                      </div>
                    <!-- end of governor results  -->
             
                </div>
                <div class="tab-pane fade" id="viewsenatorvotes" style="margin-top: -1%;">
                        <!-- senator results panel -->
                   <div class="btn btn- well pull-left"  style="color:black;width: 100%;min-width:240px;margin-left: 10px;padding: 3px;">
                         <div class="panel">
                       <h5 style="text-align: center;font-size: 20px;" >Senatorial Results  <h class="pull-right" style="margin-top: -0.5%;padding-right: 2px;"> <div class="pull-right" >
                        <a href="printpresident34B.php" class="" style="font-size: 10px;margin: 0px;" ><button class="btn btn-large btn-info printpresident box-title glyphicon glyphicon-print bg-aqua" style="margin: 0px;"><h style="color: yellow;"><b> Print </b></h></button> </a>
                     
                    </div></h></h5>
                     <div class="senatorpollingresults" id="senatorpollingresults">
                                  No Voting Activity Detected
                              </div>
                       </div>
                      </div>
                    <!-- end of senator results  -->
           
                </div>
                <div class="tab-pane fade" id="viewwomenrepvotes" style="margin-top: -1%;">
                 <!-- womenrep results panel -->
                    <div class="btn btn- well pull-left"  style="color:black;width: 100%;min-width:240px;margin-left: 10px;padding: 3px;">
                         <div class="panel">
                       <h5 style="text-align: center;font-size: 20px;" >Women-Rep Results  <h class="pull-right" style="margin-top: -0.5%;padding-right: 2px;"> <div class="pull-right" >
                        <a href="printpresident34B.php" class="" style="font-size: 10px;margin: 0px;" ><button class="btn btn-large btn-info printpresident box-title glyphicon glyphicon-print bg-aqua" style="margin: 0px;"><h style="color: yellow;"><b> Print </b></h></button> </a>
                     
                    </div></h></h5>
                      <div class="womenreppollingresults" id="womenreppollingresults">
                                  No Voting Activity Detected
                              </div>
                      </div>
                      </div>
                     <!-- end of womenrep results  -->
                   
                </div>
                 <div class="tab-pane fade" id="viewmpvotes" style="margin-top: -1%;">
                 
                    <!-- mp results panel -->
                    <div class="btn btn- well pull-left"  style="color:black;width: 100%;min-width:240px;margin-left: 10px;padding: 3px;">
                         <div class="panel">
                       <h5 style="text-align: center;font-size: 20px;" >Member of Parliament Results  <h class="pull-right" style="margin-top: -0.5%;padding-right: 2px;"> <div class="pull-right" >
                        <a href="printpresident34B.php" class="" style="font-size: 10px;margin: 0px;" ><button class="btn btn-large btn-info printpresident box-title glyphicon glyphicon-print bg-aqua" style="margin: 0px;"><h style="color: yellow;"><b> Print </b></h></button> </a>
                     
                    </div></h></h5>
                       <div class="mppollingresults" id="mppollingresults">
                                  No Voting Activity Detected
                              </div>
                       
                      </div>
                      </div>
                    
                   <!-- end of mp results  -->
                   
                </div>
                <div class="tab-pane fade" id="viewmcavotes" style="margin-top: -1%;">
           <!-- mca results panel -->
                       <div class="btn btn- well pull-left"  style="color:black;width: 100%;min-width:240px;margin-left: 10px;padding: 3px;">
                         <div class="panel">
                      <h5 style="text-align: center;font-size: 20px;" >MCA Results  <h class="pull-right" style="margin-top: -0.5%;padding-right: 2px;"> <div class="pull-right" >
                        <a href="printpresident34B.php" class="" style="font-size: 10px;margin: 0px;" ><button class="btn btn-large btn-info printpresident box-title glyphicon glyphicon-print bg-aqua" style="margin: 0px;"><h style="color: yellow;"><b> Print </b></h></button> </a>
                     
                    </div></h></h5>
                      <div class="mcapollingresults" id="mcapollingresults">
                                  No Voting Activity Detected
                              </div>
                    </div>
                      </div>
                </div>
            
            </div>
          </div>
                <!-- lki -->
                  
                      <script type="text/javascript">
 
           //load presidentila results
     $.ajax({
        url:"load_pollingpresident_votes.php",
        method:"POST",
        success:function(data){
          $('#presidentialpollingresults').html(data);
        }
      });
      //load governor results
     $.ajax({
        url:"load_pollinggovernor_votes.php",
        method:"POST",
        success:function(data){
          $('#governorpollingresults').html(data);
        }
      });
      //load senator results
       $.ajax({
        url:"load_pollingsenator_votes.php",
        method:"POST",
        success:function(data){
          $('#senatorpollingresults').html(data);
        }
      });
      //load womenrep results
      $.ajax({
        url:"load_pollingwomenrep_votes.php",
        method:"POST",
        success:function(data){
          $('#womenreppollingresults').html(data);
        }
      });
      //load mp results
      $.ajax({
        url:"load_pollingmp_votes.php",
        method:"POST",
        success:function(data){
          $('#mppollingresults').html(data);
        }
      });
      //load mca results
      $.ajax({
        url:"load_pollingmca_votes.php",
        method:"POST",
        success:function(data){
          $('#mcapollingresults').html(data);
        }
      });
                      </script>