...
<!-- retreive returnings information -->
  <?php
        include '../db_config/connection.php';

       $sql = "SELECT * FROM returnings";
...
echo "No returnings Found";
    ...
    <?php
        ...
        $sql = "SELECT * FROM returnings ";
            ...
            <form action="" method="POST" enctype="multipart/form-data" name="updatereturningsform" id="updatereturningsform">
      <input type="file" name="f1" accept="image/*" onchange="loadFile(event)" required>   <img id="output" width="100px" height="50px"/><br>
      <input type="hidden" name="nid" value="'.$nid.'">
           <button type="submit" class="btn btn-danger glyphicon glyphicon-send" name="upphoto" id="sendEmail"style="float:right;"> Update returnings Photo
...
<script type="text/javascript">
    $("#saveupdatereturnings").click(function(){
  ...
  $.ajax({
      url:"update_returnings.php",
      type:"POST",
      data:{email:email,uname:uname,upreturnings:upreturnings,nid:nid,phone:phone},
      dataType:"text",
      success:function(data){
        $('#replaymsg').html(data);
        $("#servermodallabel").html("AMVS Update returnings Success");
        $("#serverreplay").modal('show');
    ...
    //update returnings photo
    $("#updatereturningsform").on('submit',(function(e){
      e.preventDefault();
     //   var data=new FormData(this);
      $('#serverreplay').modal({"backdrop":"static"});
     
$.ajax({
      url:"update_returnings_photo.php",
      type:"POST",
      ...
      success:function(data){
        $('#replaymsg').html(data);
        $("#servermodallabel").html("AMVS Update returnings Photo Success");
        $("#serverreplay").modal('show');
        $("#serverreplay").modal('show');
      }
    });   
