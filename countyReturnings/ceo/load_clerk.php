...
//display Presidings
  ...
  if(mysqli_num_rows($result)>0){
    ...
    <caption>Clerks Officer Information</caption><thead><tr>
          <th>ID</th>
          <th>Username</th>
          <th>National ID</th>
          <th>Phone</th>
    ...
    while($row=mysqli_fetch_array($result)){
      ...
      <td>'.$row['gender'].'</td>
      <td>'.$row['polling'].'</td>
      <td colspan="3"><a title="Edit '.$row["uname"].'" class="btn btn-block btn-primary btn-xs editbtn" title="Click here to Edit Presiding '.$row["uname"].'" data-id1="'.$row["id"].'" data-id2="'.$row["uname"].'" onclick="btnc('.$row["id"].');" ><i class="fa fa-edit">Edit</i></a> 
        <a title="Delete '.$row["uname"].'" class="btn btn-block btn-danger btn-xs" href="delete_presd.php?ref='.$row["id"].'"><i class="fa fa-trash-o">Delete</i></a><button onclick="printme()">Print</button></td>
      </tr>';
    }
    $output.='</tbody>
