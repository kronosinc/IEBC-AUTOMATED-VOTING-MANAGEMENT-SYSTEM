...
//display Presidings
  ...
  if(mysqli_num_rows($result)>0){
    ...
    <caption>Presiding Officer Information</caption><thead><tr>
      ...
      <td>'.$row['gender'].'</td>
      <td>'.$row['polling'].'</td>
      <td colspan="3"><a title="Edit '.$row["uname"].'" class="btn btn-block btn-primary btn-xs editbtn optionspre" title="Click here to Edit Presiding '.$row["uname"].'" data-id1="'.$row["id"].'" data-id2="'.$row["uname"].'" onclick="btnc('.$row["id"].');" ><i class="fa fa-edit">Edit</i></a> 
        <a title="Delete '.$row["uname"].'" class="btn btn-block btn-danger btn-xs optionspre" href="delete_presd.php?ref='.$row["id"].'"><i class="fa fa-trash-o">Delete</i></a><button onclick="printme()">Print</button></td>
      </tr>';
    }
    $output.='</tbody>
