...
//display Presidings
  $select="select * from returnings  ORDER BY id";
  $result=mysqli_query($conn,$select);
  if(mysqli_num_rows($result)>0){
    $output='';
    $output.='<table border="1" class=" table table-bordered table-condensed table-responsive" id="example">
    <caption>ALL CONSTITUENCY RETURNING OFFICERS <span class="badge pull-right">'.mysqli_num_rows($result).'</span> </caption><thead><tr>
    <th>SNo</th>
          <th>Name</th>
          <th>NID</th>
          <th>Email</th>
