...
<head>
  ...
  <title>AMVS | County-Ret Custom</title>
...
<div class="wrapper">
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu">
        ...
        <li class="treeview alert alert-danger" style="margin-left:4%;margin-right:4%;font-size:15px;" >
          <a href="view_countyret.php"style="text-decoration:none;">
            <i class="fa fa-user"></i>
        ...
        <li class="treeview alert alert-success" style="margin-left:4%;margin-right:4%;font-size:15px;" >
          <a href="view_presd.php"style="text-decoration:none;">
            <i class="fa fa-user"></i>
  ...
  <div class="content-wrapper"style="background-color:#ffff99;">
    <section class="content-header">
      <h1>
        <p class="glyphicon glyphicon-upload" style="font-size:18px;color:black;"></p><b><i> <h style="font-size:20px;color:green;">Uploaded County Returnings</h></b></i>
      </h1>
      <ol class="breadcrumb">
        <li><a href="./"><i class="fa fa-dashboard btn btn-primary disabled glyphicon glyphicon-home"> Home</i> </a></li>
        <li><a href=""><i class=" btn btn-primary active"> Uploaded County Returnings</i> </a></li>
      </ol>
    </section>
    ...
    <section class="content">
      ...
      <div class="row">
        <section class="col-lg-12">
          ...
          <div class="box box-info bg-gray">
            <div class="box-header">
              ...
              <h3 class="box-title glyphicon glyphicon-file"></h3><b>County Returning Officers found on Database</b>

              <h3 class="box-title glyphicon glyphicon-search" style="margin-left:48%;"></h3>
		...
		<table class="table" id="myDisplay">
           ...
           <th>Options</th>
              ...
              <tbody>
			   ...
			   $sql = "SELECT * FROM returnings  ORDER BY id limit $page1,5";
             $result = $conn->query($sql);
			...
			echo '<tr><td>' . $row["id"]. '</td><td>' . $row["uname"]. '</td><td>'.$row["nid"]. '</td><td>'. $row["phone"]. '</td><td>'. $row["email"]. '</td><td>'. $row["gender"]. '</td><td>'. $row["county"]. '</td><td> <img width="60px" height="50px" src="data:image;base64,'.$row["photo"].'" /></td>';
		    print '<td><a title="Edit '.$row["uname"].'" class="btn btn-block btn-primary btn-xs" href="update_countyret.php?ref='.$row["id"].'"><i class="fa fa-edit">Edit</i></a>
			<a '; ?> 	<a onclick="return confirm('Are you sure you want to delete <?php echo $row['uname']; ?> ?');" <?php print 'title="Delete '.$row["uname"].'" class="btn btn-block btn-danger btn-xs" href="delete_countyret.php?ref='.$row["id"].'&page='.$page1.'"><i class="fa fa-trash-o">Delete</i></a>
        ...
        <?php
              ...
              <h4>No County-Ret Officers Uploaded so far.</h4>
              </div>
                <?php
               }
              $conn->close();
        ?>
			  ...
			  <?php
						 include '../db_config/connection.php';
						$sql = "SELECT * FROM returnings ORDER BY county";
$result = $conn->query($sql);
...
for ($b=1;$b<=$a;$b++)
{
	?> <li class="paginate_button"><a href="view_countyret.php?page=<?php echo $b; ?>" ><?php echo $b. " "; ?></a></li><?php
}
}
$conn->close();
						?>
  ...
  <footer class="main-footer bg-aqua bg-aqua">
