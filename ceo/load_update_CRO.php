...
//display Presidings
  ...
  if(mysqli_num_rows($result)>0){
    ...
    <caption><b>County Returning Officers Information</b></caption><thead><tr>
 <th>ID</th>
                  <th>Username</th>
                  <th>National ID</th>
          <th>Phone</th>
    ...
    while($row=mysqli_fetch_array($result)){
      ...
      <td>'.$row['gender'].'</td>
      <td>'.$row['county'].'</td>
      <td><a title="Edit '.$row["uname"].'" style="width:48%; min-width:50px;" class="btn btn-primary btn-xs editbtn" title="Click here to Edit Presiding '.$row["uname"].'" data-id1="'.$row["id"].'" data-id2="'.$row["uname"].'" ><i class="fa fa-edit">Edit</i></a> 
        <a title="Delete '.$row["uname"].'" style="width:48%; min-width:50px;" class="btn btn-danger btn-xs" href="delete_presd.php?ref='.$row["id"].'"><i class="fa fa-trash-o">Delete</i></a></td>
      </tr>';
    }
    $output.='</tbody></table>';
    echo $output;
    ...
    echo 'No County Returning Officers Found';
  }
