...
//display Presidings
  $select="select * from returnings  ORDER BY county";
  ...
  if(mysqli_num_rows($result)>0){
    ...
    while($row=mysqli_fetch_array($result)){
      ...
      <td colspan="3"><a title="Edit '.$row["uname"].'" style="width:48.5%;min-width:80px;" class="btn btn-primary btn-xs editbtn" title="Click here to Edit Presiding '.$row["uname"].'" data-id1="'.$row["id"].'" data-id2="'.$row["uname"].'" onclick="btnc('.$row["id"].');" ><i class="fa fa-edit">Edit</i></a> 
        <a title="Delete '.$row["uname"].'" style="width:48.5%;min-width:80px;" class="btn btn-danger btn-xs" href="delete_presd.php?ref='.$row["id"].'"><i class="fa fa-trash-o">Delete</i></a></td>
      </tr>';
    }
    $output.='</tbody>
    ...
    echo 'No Constituency Returning Officers Found';
  }
