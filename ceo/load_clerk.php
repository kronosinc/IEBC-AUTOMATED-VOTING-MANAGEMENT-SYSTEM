...
//display Presidings
  $select="select * from clerk  ORDER BY id";
  $result=mysqli_query($conn,$select);
  if(mysqli_num_rows($result)>0){
    $output='';
    $output.='<table border="1" class=" table table-stripped table-condensed table-responsive" id="example" style="padding: 2px;">
    <caption><b>Clerk Officers Information</b></caption><thead><tr>
          <th>ID</th>
          <th>Username</th>
          <th>National ID</th>
          <th>Phone</th>
    ...
    while($row=mysqli_fetch_array($result)){
      ...
      <td>'.$row['gender'].'</td>
      <td>'.$row['polling'].'</td>
      <td colspan="3"><a title="Edit '.$row["uname"].'" style="width:40%;min-width:40px;" class="btn btn-primary btn-xs editbtn" title="Click here to Edit Presiding '.$row["uname"].'" data-id1="'.$row["id"].'" data-id2="'.$row["uname"].'" onclick="btnc('.$row["id"].');" ><i class="fa fa-edit">Edit</i></a> 
        <a title="Delete '.$row["uname"].'" style="width:40%;min-width:40px;" class="btn btn-block btn-danger btn-xs" href="delete_presd.php?ref='.$row["id"].'"><i class="fa fa-trash-o">Delete</i></td>
      </tr>';
    }
    $output.='</tbody>
...
//         $("#closeokay").show();
//         $('#replaymsg').html(data);
//         $("#servermodallabel").html("AMVS Presiding Edit");
//         $("#serverreplay").modal('show');
//       }
//     });
