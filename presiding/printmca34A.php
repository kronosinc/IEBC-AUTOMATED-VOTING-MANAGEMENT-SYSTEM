 ...
 $this->SetDrawColor(0,80,180);
   ...
   // $this->Cell(2);
    ...
    $this->SetFont('Times','',12);
     ...
     $this->Cell(0,20,'FORM 34A: Member of County Assemble Results ',0,1,'C');
     ...
     $this->Cell(10,10,'Sno',1,0,'C');
       ...
       $this->SetTextColor(0,255,0);
     $this->SetFont('Times','',13);
     $this->Cell(70,10,'MCA Name',1,0,'C');
             ...
             if ($result1->num_rows > 0) {
              while($row1 = $result1->fetch_assoc()) {
                $pres=$row1['contestant'];
                   $sql21 = "SELECT mca.Sno,mca.uname,mca.partycode,party.partycode,party.partyname FROM `mca`,party WHERE mca.Sno='$pres' && mca.partycode=party.partycode order by mca.Sno ";
                      ...
                      if(mysqli_num_rows($resultvote)>0){
                        while($row21 =mysqli_fetch_array($resultvote)) {
                           ...
                           $counts++;
                              ...
                              $pdf->Cell(100,10,'MCA not Found ',1,1,'C');
                       ...
                       $pdf->Cell(100,10,'No Vote For Any MCA '.mysqli_error($connection),1,1,'C');
                           
                         }
 // $sql1 = "SELECT max(votes) as winner,type,contestant FROM `presiding_results` where polling='$currentpolling' && type='MCA'";
 //             $result1 = $conn->query($sql1);
 //             if ($result1->num_rows > 0) {
 //              while($row1 = $result1->fetch_assoc()) {
 //                $pres=$row1['contestant'];
