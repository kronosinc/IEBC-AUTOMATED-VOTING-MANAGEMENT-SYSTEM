 ...
 $this->SetDrawColor(0,80,180);
   ...
   // $this->Cell(2);
    ...
    $this->SetFont('Times','',20);
     ...
     $this->Cell(0,20,'FORM 34A: Presidential Results ',0,1,'C');
...
if (isset($_SESSION['loggedin']) && isset($_SESSION['presiding_username'])  && $_SESSION['loggedin'] == true) {
 $myusername = $_SESSION['presiding_username'];
  ...
  header("Location:../index.php?login_err=Login First");
    ...
    //select applicant info
      $counts=1;
          $sql1 = "SELECT * FROM `presiding_results` where polling='$currentpolling' && type='President'";
             $result1 = $conn->query($sql1);
             if ($result1->num_rows > 0) {
              while($row1 = $result1->fetch_assoc()) {
                $pres=$row1['contestant'];
                   $sql21 = "SELECT president.Sno,president.uname,president.partycode,party.partycode,party.partyname FROM `president`,party WHERE president.Sno='$pres' && president.partycode=party.partycode order by president.Sno ";
 ...
 // $sql1 = "SELECT max(votes) as winner,type,contestant FROM `presiding_results` where polling='$currentpolling' && type='President'";
 //             $result1 = $conn->query($sql1);
 //             if ($result1->num_rows > 0) {
 //              while($row1 = $result1->fetch_assoc()) {
 //                $pres=$row1['contestant'];
                   $sql21 = "SELECT president.Sno,president.uname,president.partycode,party.partycode,party.partyname,presiding_results.type,max(presiding_results.votes) as winner,presiding_results.contestant FROM `president`,party,presiding_results WHERE  president.partycode=party.partycode && polling='$currentpolling' && type='President' order by president.Sno ";
                      $resultvote=mysqli_query($conn,$sql21);
