...
//Check if voter has already voted for the president
mysqli_autocommit($conn,false);


$sql1 = "SELECT * FROM votes where nid='$nid' and polling ='$pollingstation' and president=0";
$resultvoter=mysqli_query($conn,$sql1);
if(mysqli_num_rows($resultvoter)>0){
    while($row = mysqli_fetch_array($resultvoter)) {
         $sql = "UPDATE votes SET president='$presidentid' WHERE nid='$nid'  and polling ='$pollingstation'";
        if ($conn->query($sql) === TRUE) {
              //update polling station votes
                   $sql2 = "SELECT * FROM presiding_results where contestant='$presidentid' and type='President' and polling ='$pollingstation'";
                    ...
                    while($row2 = mysqli_fetch_array($presvoter)) {
                      ...
                      $updatevotes=$curvotes+1;
                         $sql = "UPDATE presiding_results SET votes='$updatevotes' WHERE contestant='$presidentid' and type='President' and polling ='$pollingstation'";
                          if ($conn->query($sql) === TRUE) {
                             
                              //update ward votes
                                       $sqlward = "SELECT * FROM ward_results where contestant='$presidentid' and type='President' and ward ='$currentwardlogin'";
                                        ...
                                        while($rowward = mysqli_fetch_array($presvoter)) {
                                          ...
                                          $updatevotesward=$curvotesward+1;
                                             $sql = "UPDATE ward_results SET votes='$updatevotesward' WHERE contestant='$presidentid' and type='President' and ward ='$currentwardlogin'";
                                              if ($conn->query($sql) === TRUE) {
                                                   //update constituency votes
                                                         $sqlconstituency = "SELECT * FROM constituency_results where contestant='$presidentid' and type='President' and constituency ='$currentconstituencylogin'";
                                                          ...
                                                          while($rowconstituency = mysqli_fetch_array($presvoter)) {
                                                            ...
                                                            $updatevotesconstituency=$curvotesconstituency+1;
                                                               $sql = "UPDATE constituency_results SET votes='$updatevotesconstituency' WHERE contestant='$presidentid' and type='President' and constituency ='$currentconstituencylogin'";
                                                                if ($conn->query($sql) === TRUE) {
                                                                         //update county votes
                                                                       $sqlcounty = "SELECT * FROM county_results where contestant='$presidentid' and type='President' and county ='$currentcountylogin'";
                                                                        ...
                                                                        while($rowcounty = mysqli_fetch_array($presvoter)) {
                                                                          ...
                                                                          $updatevotescounty=$curvotescounty+1;
                                                                             $sql = "UPDATE county_results SET votes='$updatevotescounty' WHERE contestant='$presidentid' and type='President' and county ='$currentcountylogin'";
                                                                              if ($conn->query($sql) === TRUE) {
            ...
            mysqli_rollback($conn);
             echo "Failed to Vote Selected President .Please Select and Vote Again";
          }
    }
}
else{
