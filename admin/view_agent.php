...
<head>
  ...
  <title>AMVS | Agents Custom</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper"style="background-color:#ffff99;">
    <section class="content-header">
      <h1>
        <p class="glyphicon glyphicon-upload" style="font-size:18px;color:black;"></p><b><i> <h style="font-size:20px;color:green;">Uploaded Agents</h></b></i>
      </h1>
      <ol class="breadcrumb">
        <li><a href="./"><i class="fa fa-dashboard btn btn-primary disabled glyphicon glyphicon-home"> Home</i> </a></li>
        <li><a href=""><i class=" btn btn-primary active"> Uploaded Agents</i> </a></li>
      </ol>
    </section>
    ...
    <section class="content">
        ...
        <section class="col-lg-12">
          ...
          <div class="box box-info bg-gray">
            <div class="box-header">
              ...
              <h3 class="box-title glyphicon glyphicon-file"></h3><b>Agents Officers found on Database</b>
      ...
      <th>Options</th>
			   ...
			   $sql = "SELECT * FROM agent  ORDER BY polling limit $page1,5";
             $result = $conn->query($sql);
			...
			echo '<tr><td>' . $row["id"]. '</td><td>' . $row["uname"]. '</td><td>'.$row["nid"]. '</td><td>'. $row["phone"]. '</td><td>'. $row["email"]. '</td><td>'. $row["gender"]. '</td><td>'. $row["polling"]. '</td><td> <img width="60px" height="50px" src="data:image;base64,'.$row["photo"].'" /></td>';
		    print '<td><a title="Edit '.$row["uname"].'" class="btn btn-block btn-primary btn-xs" href="update_agent.php?ref='.$row["id"].'"><i class="fa fa-edit">Edit</i></a>
			<a '; ?> 	<a onclick="return confirm('Are you sure you want to delete <?php echo $row['uname']; ?> ?');" <?php print 'title="Delete '.$row["uname"].'" class="btn btn-block btn-danger btn-xs" href="delete_agent.php?ref='.$row["id"].'&page='.$page1.'"><i class="fa fa-trash-o">Delete</i></a>
            ...
            <?php
                  ...
                  <h4>No Agent Officers Uploaded so far.</h4>
                  </div>
                    <?php
                   }
                  $conn->close();
            ?>
			  ...
			  <?php
						 include '../db_config/connection.php';
						$sql = "SELECT * FROM agent ORDER BY county";
$result = $conn->query($sql);
...
for ($b=1;$b<=$a;$b++)
{
	?> <li class="paginate_button"><a href="view_agent.php?page=<?php echo $b; ?>" ><?php echo $b. " "; ?></a></li><?php
}
}
$conn->close();
						?>
