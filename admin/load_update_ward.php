...
//display counties
  ...
  if(mysqli_num_rows($result)>0){
    ...
    <caption><b>Wards Information</b></caption><thead><tr>
      ...
      <td>'.$row['wardcode'].'</td>
      <td>'.$row['wardname'].'</td>
      <td colspan="3"><a title="Edit '.$row["wardname"].'"style="width:49.5%;min-width:150px;" class="btn btn-large btn-primary btn-xs editbtn optionspre" title="Click here to Edit  '.$row["wardname"].'" data-id1="'.$row["wardcode"].'" data-id2="'.$row["wardname"].'" onclick="btnc('.$row["wardcode"].');" ><i class="fa fa-edit">Edit</i></a> 
        <a title="Delete '.$row["wardname"].'" style="width:49.5%;min-width:150px;" class="btn btn-large btn-danger btn-xs optionspre"><i class="fa fa-trash-o">Delete</i></a></td>
      </tr>';
    }
    $output.='</tbody>
