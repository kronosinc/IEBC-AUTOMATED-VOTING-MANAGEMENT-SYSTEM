...
<div class="wrapper">
  ...
  <div class="content-wrapper"style="background-color:#ffff99;">
    <section class="content-header">
      <h1>
          <p class="glyphicon glyphicon-upload" style="font-size:18px;color:black;"></p><b><i> <h style="font-size:20px;color:green;">Uploaded Wards</h></b></i>
      </h1>
      <ol class="breadcrumb">
        <li><a href="./"><i class="fa fa-dashboard btn btn-primary disabled glyphicon glyphicon-home"> Home</i> </a></li>
        <li><a href=""><i class=" btn btn-primary active"> Uploaded Wards</i> </a></li>
      </ol>
    </section>
    ...
    <section class="content">
      ...
      <div class="row">
        <section class="col-lg-12 ">
          ...
          <div class="box box-info bg-gray">
            <div class="box-header">
              ...
              <h3 class="box-title glyphicon glyphicon-file"></h3><b>Wards found on Database</b>

              <h3 class="box-title glyphicon glyphicon-search" style="margin-left:60%;"></h3>
              <input style="float:right;background-color:white;border-radius:12px 12px 12px 12px;border:2px solid red;text-align:center;" type="text" id="search" name="term" placeholder="Filter by Ward Name"maxlength="8" onkeyup="searchJosshua()">
		...
		<table class="table" id="myDisplay">
               ...
               <tbody>
                 <?php
        			   ...
        			   $sql = "SELECT * FROM ward  ORDER BY wardcode limit $page1,5";
                     $result = $conn->query($sql);

                     if ($result->num_rows > 0) {

                      while($row = $result->fetch_assoc()) {
			echo "<tr><td>" . $row["Sno"]. "</td><td>" . $row["wardcode"]. "</td><td>". $row['wardname']. "</td><td>". $row['constituencycode']. "</td>";
		    print '<td><a title="Edit '.$row["wardname"].'" class="btn btn-block btn-primary btn-xs" href="update_ward.php?ref='.$row["Sno"].'"><i class="fa fa-edit">Edit</i></a>
			<a '; ?> 	<a onclick="return confirm('Are you sure you want to delete <?php echo $row['wardname']; ?> ?');" <?php print 'title="Delete '.$row["wardname"].'" class="btn btn-block btn-danger btn-xs" href="delete_ward.php?ref='.$row["id"].'&page='.$page1.'"><i class="fa fa-trash-o">Delete</i></a>
        ...
        Registered student will be shown here
			  ...
			  <?php
						 include '../db_config/connection.php';
						$sql = "SELECT * FROM ward  ORDER BY wardcode ";
$result = $conn->query($sql);
...
for ($b=1;$b<=$a;$b++)
{
	?> <li class="paginate_button"><a href="view_wards.php?page=<?php echo $b; ?>" ><?php echo $b. " "; ?></a></li><?php
}
}
$conn->close();
						?>
